...
public class UniWebView : MonoBehaviour {
    ...
    private void WebViewDone(string message) {
        ...
        if (destroy) {
            ...
            // Stop TTS
            GameObject.Find("Scripts").GetComponent<TextToSpeech>().ExitTTS();

            // Loading previous scene
            GameObject.Find("SceneControl").GetComponent<LoadingScenes>().LoadScene(1);           
    ...
    private IEnumerator LoadFromJarPackage(string jarFilePath) {
        UnityWebRequest stream = new UnityWebRequest(jarFilePath);
        ...
        if (stream.error != null) {
            ...
            LoadHTMLString(stream.downloadHandler.text, "");
        }
    }
